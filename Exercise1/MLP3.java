...
public class MLP3 {
    ...
    private String[] activations =  new String[4];
    private double[][] trainingSet;
    private double[][] testSet;
    private ArrayList<double[][]> newTrainingSet = new ArrayList<>();
    private double[][][] weights; 
    private double[][] biases;
    private double[][][] derivativeOfWeights = new double [4][][];
    private double[][] derivativeOfBiases = new double [4][]; 
    ...
    public void getDataSets(){
   	...
   	public void weightInitialization(){
    	...
    	double[][] weights3 = new double[H3][H2];
    	double[][] weightsK = new double[K][H3];
   		...
   		for (int i = 0; i < H3; i++){
   			for (int j = 0;j < H2; j++){
   				weights3[i][j]= random.nextDouble() * 2 - 1;
   			}
   		}
   		for (int i = 0; i < K; i++){
   			for (int j = 0;j < H3; j++){
   	...
   	public void biasInitialization(){
     	...
     	double[] biasK = new double[K];
   		...
   		for (int i = 0; i < H3; i++){
   			bias3[i] = random.nextDouble() * 2 - 1;
   		}
   		for (int i = 0; i < K; i++){
   			biasK[i] = random.nextDouble() * 2 - 1;
   		}
   		biases = new double[][] { bias1, bias2, bias3, biasK };
   	}


   	public void forwardPass(double[] x,int D, double[] y, int K){
   		double[] input = x;
        int[] numberOfNeurons = {D,H1,H2,H3,K};
        for (int i = 1; i < 5; i++) {
    ...
    public void backprop(double[] x, int d, double[] t, int K) {
        ...
        deltas[3] = new double[K];
        for (int i = 0; i < K; i++) {
            deltas[3][i] = (inputs[4][i] - t[i]) * derivativeOfActivation(inputs[4][i],activations[3]);
        }
    ...
    public static void main(String[] args) {
        MLP3 mlp3 = new MLP3();
        mlp3.getDataSets();
        mlp3.categoryClassification();
        String[] functions = {"tanh", "tanh", "relu", "sigmoid"};
        mlp3.activations = functions;
        mlp3.gradientDescent(20); 
        mlp3.evaluateGeneralization();
    }
}